...
from packages.regression_model.config.config import DATASET_PATH
...
X3 = np.random.uniform(0,1,10000)+500
Error = np.random.uniform(0,0.1,10000)

# create label
Y = 10*X1+0.2*X2.reshape(X1.shape)+900*X3.reshape(X1.shape)+Error.reshape(X1.shape)

# create data for dataframe
data = np.concatenate((X1, X2.reshape(X1.shape), X3.reshape(X1.shape), Y.reshape(X1.shape)), axis=1)

#create data for test dataframe
